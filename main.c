...
static inline void init_nA_nB(long int * nA, long int * nB)
{
	...
	for(int j=0; j<NB_COEFF;j++){
		...
		nA[j] = nA[j]&CONV_MASK_N6_PMNS;
		nB[j] = nB[j]&CONV_MASK_N6_PMNS;
	}
...
int main(int argc, char* argv[]){
	...
	mpz_t modul_p_n5_pmns;
	mpz_t gama_pow_n5_pmns[POLY_DEG];

	mpz_t modul_p_n5_amns;
	mpz_t gama_pow_n5_amns[POLY_DEG];
	
	mpz_t modul_p_n6_pmns;
	mpz_t gama_pow_n6_pmns[POLY_DEG_N6_PMNS];
	...
	init_data_n5_pmns(modul_p_n5_pmns,gama_pow_n5_pmns);
	init_data_n5_amns(modul_p_n5_amns,gama_pow_n5_amns);
	init_data(modul_p_n6_pmns,gama_pow_n6_pmns);
	
	mpz_urandomm(A, r, modul_p_n5_pmns);
	mpz_urandomm(B, r, modul_p_n5_pmns);
	mpz_set(E, A);

	
	nb_limbs = mpz_size (modul_p_n5_pmns);
	
	p_limbs = mpz_limbs_read (modul_p_n5_pmns);   
	
	binvert_limb (mip0, p_limbs[0]);
	mip0 = -mip0;
	
	mpz_setbit (R2, 2*nb_limbs*8*sizeof(mp_limb_t)); 
	mpz_mod(R2, R2, modul_p_n5_pmns);
	...
	gmp_printf("r_lgmp  : %Zd\n", F);
		...
		mpz_mul (E, A, B);
		mpz_mod (E, E, modul_p_n5_pmns);
	gmp_printf("r_gmp   : %Zd\n", E);


	/***********************************************/
	printf("\nmult_mod_poly_AVX512 n=6, phi=2^52 :\n------------------------------------\n");
	...
	gmp_printf("A       : %Zd\n", A);
	gmp_printf("B       : %Zd\n\n", B);
	afficheVect(nA->i64,"nA",NB_COEFF_N6_PMNS);
	afficheVect(nB->i64,"nB",NB_COEFF_N6_PMNS);
	
	mult_mod_poly_AVX512(res->i64,nA->i64,nB->i64);
		
	afficheVect(res->i64,"res (AVX512)",NB_COEFF_N6_PMNS);
		
	printf("\nComparison with mult_mod_poly n=6, phi=2^52 :\n---------------------------------------------\n");
	
	mult_mod_poly(resMul->i64, nA->i64,nB->i64);

	afficheVect(resMul->i64,"resMul    ",NB_COEFF_N6_PMNS);
	
	for(int i=0; i<NB_COEFF_N6_PMNS;i++) tmp[i>>3].i64[i&0x7] = res[i>>3].i64[i&0x7]^resMul[i>>3].i64[i&0x7];
	
	printf("\n");
	afficheVect(tmp->i64,"cmp",NB_COEFF_N6_PMNS);
	
	printf("\n");
	
	for(int i=0; i<NB_COEFF_N6_PMNS;i++)
	...
	exact_coeffs_reduction(res->i64, res->i64);
	from_pmns_to_int(C, res->i64, modul_p_n6_pmns, gama_pow_n6_pmns);
	...
	printf("\nComparison with mult_mod_poly_n5 :\n----------------------------------\n");
	
	
	gmp_printf("A       : %Zd\n", A);
	gmp_printf("B       : %Zd\n\n", B);
	from_int_to_pmns_n5_pmns(nA->i64, A);
	from_int_to_pmns_n5_pmns(nB->i64, B);
	
	mult_mod_poly_n5_pmns(res->i64, nA->i64,nB->i64);

	afficheVect(nA->i64,"nA",NB_COEFF);
	afficheVect(nB->i64,"nB",NB_COEFF);
	
	afficheVect(res->i64,"res",NB_COEFF);
	
	
	exact_coeffs_reduction_n5_pmns(res->i64, res->i64);
	from_pmns_to_int_n5_pmns(R2, res->i64, modul_p_n5_pmns, gama_pow_n5_pmns);
	...
	gmp_printf("A       : %Zd\n", A);
	gmp_printf("B       : %Zd\n\n", B);
	from_int_to_amns_n5_amns(nA->i64, A);
	from_int_to_amns_n5_amns(nB->i64, B);
	
	mult_mod_poly_n5_amns(res->i64, nA->i64,nB->i64);
	...
	exact_coeffs_reduction_n5_amns(res->i64, res->i64);
	from_amns_to_int_n5_amns(R3, res->i64, modul_p_n5_amns, gama_pow_n5_amns);
	...
	gmp_printf("A       : %Zd\n", A);
	gmp_printf("B       : %Zd\n\n", B);
	gmp_printf("r_pmns (n6, AVX512) : %Zd\n", C);
	gmp_printf("r_pmns (n5)         : %Zd\n", R2);
	
	gmp_printf("r_amns (n5)         : %Zd\n", R3);
	...
	*********************/
	...
	printf("timer mult_mod_poly_n6_pmns        : %llu\n",timer/(K_T*NSAMPLES));
	...
	printf("timer mult_mod_poly_n6_pmns AVX512 : %llu\n",timer1/(K_T*NSAMPLES));	
	...
	for(int k=0; k<NSAMPLES;k++){
		...
		for(int i=0;i<NTEST;i++)
		{
			mult_mod_poly_n5_pmns(resMul->i64, nA->i64,nB->i64);
	...
	printf("timer mult_mod_poly_n5_pmns        : %llu\n",timer2/(K_T*NSAMPLES));	
	...
	for(int k=0; k<NSAMPLES;k++){
		...
		for(int i=0;i<NTEST;i++)
		{
			mult_mod_poly_n5_amns(resMul->i64, nA->i64,nB->i64);
	...
	printf("timer mult_mod_poly_n5_amns        : %llu\n",timer3/(K_T*NSAMPLES));
...
*********************************************************/
	...
	printf("#Instructions mult_mod_poly_n6_pmns        : %llu\n",timer/(K_T*NSAMPLES));
	...
	printf("#Instructions mult_mod_poly_n6_pmns AVX512 : %llu\n",timer1/(K_T*NSAMPLES));	
	
	
	// #Instructions mult_mod_poly_n5_pmns        :
	...
	printf("#Instructions mult_mod_poly_n5_pmns        : %llu\n",timer2/(K_T*NSAMPLES));	

	
	// #Instructions mult_mod_poly_n5_amns        :
	...
	printf("#Instructions mult_mod_poly_n5_amns        : %llu\n",timer3/(K_T*NSAMPLES));
...
fin:
	...
	free(bm2_limbs);
	free(mip_limbs);
	free_data_n5_pmns(modul_p_n5_pmns,gama_pow_n5_pmns);
	free_data_n5_amns(modul_p_n5_amns,gama_pow_n5_amns);
	free_data(modul_p_n6_pmns,gama_pow_n6_pmns);
