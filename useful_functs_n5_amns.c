...
void from_int_to_amns_n5_amns(int64_t *rop, mpz_t op){
	...
	while(tmp->_mp_size && (i < NB_COEFF)){
		scalar_mult_lpoly_n5_amns(tmp_poly, polys_P_n5_amns[i++], (tmp->_mp_d[0]) & CONV_MASK);
		add_lpoly_n5_amns(tmp_sum, tmp_sum, tmp_poly);

		mpz_tdiv_q_2exp (tmp, tmp, RHO_LOG2);
	}

	internal_reduction_n5_amns(rop, tmp_sum);
...
void from_amns_to_int_n5_amns(mpz_t rop, int64_t *op, mpz_t modul_p,  mpz_t gama_pow[POLY_DEG] ){
	...
	from_mont_domain_n5_amns(tmp_conv, op);
...
void init_data_n5_amns(	mpz_t modul_p, mpz_t gama_pow[POLY_DEG] ){
	...
	mpz_set_str (gama_pow[0], "90695635360428435680584672850873055410858588101238735650770860130321378755705", 10);
...
int cmp_poly_evals_n5_amns(int64_t *pa, int64_t *pb, mpz_t modul_p,  mpz_t gama_pow[POLY_DEG] ){
	...
	mpz_t a, b;
	mpz_inits (a, b, NULL);
	from_amns_to_int_n5_amns(a, pa, modul_p,  gama_pow );
	from_amns_to_int_n5_amns(b, pb, modul_p,  gama_pow );
	rep = mpz_cmp (a, b);
	mpz_clears (a, b, NULL);
	return rep;
}
